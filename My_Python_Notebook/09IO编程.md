...
在命令行下，我们可以通过输入操作系统提供的各种命令，比如dir、cp等，来操作文件和目录。这些命令的本质其实就是简单地调用了**操作系统提供的接口函数**。

那如果想在Python程序中操作文件和目录该怎么办呢？Python内置的 `os` 模块同样给与我们调用操作系统提供的接口函数的能力。
...
除了前面的 `os` 模块中，操作文件和目录的函数还有一部分放在 `os.path` 模块中。比方说用于生成绝对路径的 `abspath()` 函数：
...
这些合并、拆分路径的函数**并不要求文件和目录真实存在**，它们只是对字符串进行操作。
